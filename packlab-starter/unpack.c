...
int main(int argc, char* argv[]) {
  ...
  // Parse the header to determine what the configuration should be
  parse_header(input_data, input_len, &config);

  // Create a buffer containing the file data only
  if (config.data_offset > input_len) {
    error_and_exit("ERROR: input file is shorter than expected\n");
  }
  size_t data_len = input_len - config.data_offset;
  uint8_t* data = malloc_and_check(data_len);
  memcpy(data, &(input_data[config.data_offset]), data_len);

  // Done with the raw input data
  free(input_data);

  // Handle checksumming
  if (config.should_checksum) {
